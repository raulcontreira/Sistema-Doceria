 ...
 * (somente as com status CONCLUIDA contam para faturamento). Para o
 * volume de dados de um projeto didatico, agregar em memoria (em vez
 * de escrever uma consulta SQL de agregacao) e simples e suficiente;
 * em um sistema com milhoes de vendas, o ideal seria mover esse calculo
 * para uma consulta JPQL com {@code GROUP BY} no banco.
 */
@Service
public class RelatorioService {
    ...
    public ResumoVendas gerarResumoGeral() {
        return montarResumo(filtrarConcluidas(vendaService.listarTodos()));
    }
    ...
    public ResumoVendas gerarResumoPorPeriodo(LocalDateTime inicio, LocalDateTime fim) {
        return montarResumo(filtrarConcluidas(vendaService.buscarPorPeriodo(inicio, fim)));
    }
    ...
    private ResumoVendas montarResumo(List<Venda> vendas) {
        ...
        Map<String, int[]> quantidadePorProduto = new LinkedHashMap<>();
        Map<String, double[]> faturamentoPorProduto = new LinkedHashMap<>();
